...
instance APIFacade CMDAPI where
        ...
        build1 _ r bf fp= runAPI r "build1" ["--file="++fp,"--buildflags="++(show bf)]
        getBuildFlags _ r fp= runAPI r "getbuildflags" ["--file="++fp]
        getOutline _ r bf fp= runAPI r "outline" ["--file="++fp,"--buildflags="++(show bf)]
        getTokenTypes _ r fp= runAPI r "tokentypes" ["--file="++fp]
        getOccurrences _ r bf fp s= runAPI r "occurrences" ["--file="++fp,"--buildflags="++(show bf),"--token="++s]
        getThingAtPoint _ r bf fp l c q t= runAPI r "thingatpoint" ["--file="++fp,"--buildflags="++(show bf),"--line="++(show l),"--column="++(show c),"--qualify="++(show q),"--typed="++(show t)]
